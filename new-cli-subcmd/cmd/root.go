...
import (
	...
	"new-cli-subcmd/cmd/get"
	"new-cli-subcmd/cmd/new"
...
func addSubCommands() {
	RootCmd.AddCommand(
		// from 'import new-cli-subcmd/cmd/<subcommand:package>'
		// <package>.InitSubCommands(c),
		get.InitSubCommands(c),
		new.InitSubCommands(c),
	)
}
	...
	if cfgFile != "" {
		...
		if _, err := os.Stat(workDir); err != nil {
			if os.IsNotExist(err) {
				...
				if mkerr != nil {
					logrus.Fatal("Error creating ~/.config/new-cli-subcmd directory", mkerr)
				}
			}
		}
		if stat, err := os.Stat(workDir); err == nil && stat.IsDir() {
			...
			logrus.Info("The ~/.config/new-cli-subcmd path is a file and not a directory, please remove the 'new-cli-subcmd' file.")
			os.Exit(1)
		}
	}
